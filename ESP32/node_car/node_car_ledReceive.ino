...
void handlePublish(uint16_t msgId, uint8_t qos, String topic, uint8_t* payload, int len) {
    ...
    } else if (topic == "telemetry/signal_state" && len >= 2) {
    uint8_t sb      = payload[0];
    uint8_t remSec  = payload[1];

    uint8_t car    = sb & 0x03;
    uint8_t ped    = (sb >> 2) & 0x03;
    bool pedExt    = (sb >> 4) & 0x01;
    bool emerg     = (sb >> 5) & 0x01;

    const char* carStr[] = {"GREEN","YELLOW","RED"};
    const char* pedStr[] = {"WALK","RED"};

    Serial.println("── signal_state ──");
    Serial.printf("  CAR=%s  PED=%s  pedExt=%d  emerg=%d  rem=%ds\n",
        car<3?carStr[car]:"?", ped<2?pedStr[ped]:"?",
        pedExt, emerg, remSec);

    // 차량 제어 로직
    if (emerg) {
        setTargetSpeed(0.0f, MQTT_CONTROL);
        Serial.println("  → EMERGENCY STOP");
    } else if (car == 2 && ped == 0) {
        // 차 빨강 + 보행자 WALK → 정지 유지
        setTargetSpeed(0.0f, MQTT_CONTROL);
        Serial.println("  → 보행자 WALK: 정지 유지");
    }
    // (추후 감속 로직 추가 가능)

    } else if (topic == "telemetry/signal_end") {
    Serial.println("[signal] 세션 종료 수신");
    // 필요하면 HTTP_CONTROL 복귀 등 처리
    }
    else {
...
void setup() {
    ...
    subscribe("telemetry/signal_state");
    subscribe("telemetry/signal_end");
